        ...
        future.onComplete {
          case Success(res) =>
            val action = for {
              ...
              token <- doc.getAs[String]("adminToken")
           } yield (pwd == DigestUtils.sha1Hex(auth.passw), token)

           action match {
             case Some((matched, token)) if matched =>
               writer.acquireAndGet(
                 _.print(ArgJson("token" -> jString(token)).nospaces)
               )
             case _ => env.jsonError(400, "Authentication mismatch")
            }
